...
class DBVSR(nn.Module):
    ...
    def forward(self, x, x_bicubic, kernel):
        ...
        kernel_output_shift = kernel_shift(kernel_output_shift.detach().cpu().numpy())
        kernel_output_shift = torch.FloatTensor(kernel_output_shift).cuda()
        ...
        # deconv
        mid_deconv = self.deconv_layer(x_mid_bicubic, kernel_output_shift.detach())
        ...
        mid_deconv_depth = self.spatial2depth(mid_deconv, scale=self.scale)
